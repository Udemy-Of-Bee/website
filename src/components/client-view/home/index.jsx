...
const ClientHomeView = ({ data }) => {
    // Đoạn này sử dụng useMemo nghĩa là hàm variants chỉ được gọi một lần khi component được mount
    const setVariants = useMemo(() => variants(), []);

    // Giới hạn vùng kéo của ảnh bên phải
                            ...
                            {data && data.length
                                ? data[0]?.heading
                                      ...
                                      .map((item, index) => (
                                          <span
                                              key={index}
                                              className={`${index === 2 || index === 3 ? 'text-green-main' : ' text-gray-800'}`}
                                          >
                                              {item}{' '}
                                          </span>
                                      ))
